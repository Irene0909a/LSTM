...
library(ggplot2)
library(stringr)
setwd("C:\\Users\\irene1017a\\OneDrive\\文件\\析數")
orders <- getwd <- read.csv('orders.csv')
...
orders.segm <- orders %>%
  ...
  # 把商品放入變數中
  mutate(cart=paste(ifelse(orders$product['water']!=0, '、瓶裝水', ''),
                    ifelse(orders$product['bread']!=0, '、牛奶麵包', ''),
                    ifelse(orders$product['Cabbage']!=0, '、高麗菜', ''), sep='')) %>%
  arrange(clientId)
